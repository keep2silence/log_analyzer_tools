...
while (<FILE>) {
	...
	if ($all[15] >= $UP_OFFSET_PROB) {
		if ($net_posi < 0) {
			...
			$profit = $sell_open_price - $all[9];	
			$sum_profit += $profit;
			$line = $line . ",$all[9]up_offset$sell_open_price,$profit,$sum_profit"; 
			++$net_posi;
		}
	}
	...
	if ($all[15] <= $DOWN_OFFSET_PROB) {
		if ($net_posi > 0) {
			...
			$profit = $all [7] - $buy_open_price;	
			$sum_profit += $profit;
			$line = $line . ",$all[7]down_offset$buy_open_price,$profit,$sum_profit";
			--$net_posi;
		}
	}
